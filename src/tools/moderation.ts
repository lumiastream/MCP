...
export function registerModeration(server: McpServer, client: LumiaClient): void {
	server.registerTool(
		'moderate_user',
		{
			...
			description: `Ban, unban, or time out a user, or grant/remove VIP. Ban/unban/timeout work on Twitch and YouTube; VIP is Twitch only. For "timeout", set duration_seconds and optionally a reason.`,
			inputSchema: {
				...
				duration_seconds: z.number().int().min(1).default(10).describe('For "timeout": how long the timeout lasts, in seconds — 600 is 10 minutes. Defaults to 10.'),
				reason: z.string().optional().describe('For "timeout": the reason to show.'),
			},
			annotations: { readOnlyHint: false, destructiveHint: true, openWorldHint: true },
		},
		async ({ action, username, platform, duration_seconds, reason }) => {
			try {
